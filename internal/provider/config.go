...
func (pc *Configurator) applyEnvironmentConfig(
	...
	if envConfig.EndpointURL != "" {
		if endpoints, ok := config["endpoints"].(map[string]interface{}); !ok || len(endpoints) == 0 {
...
func (pc *Configurator) configureEndpoints(
	...
	// Set endpoints block
	// AWS provider v6 expects endpoints as a direct map
	...
	return map[string]Defaults{
		"aws": {
			...
			EndpointServices: []string{
				// Services from LocalStack documentation
